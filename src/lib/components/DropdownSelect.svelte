...
<div class="relative" bind:this={root} use:clickOutside>
	...
	<button
		...
		class="flex w-full items-center justify-between rounded-full border border-zinc-300 bg-white px-4 py-2 pr-10
           ...
           focus:ring-2 focus:ring-[color:var(--brand)] focus:outline-none
	...
	>
		...
		<svg
			class="ml-2 h-5 w-5 text-zinc-500 transition"
		...
		>
			{#each options as opt, i}
				<li role="option" aria-selected={opt === value}>
                   ...
                   {opt === value
							? 'bg-[color:var(--brand)]/10 text-[color:var(--brand)] dark:bg-[color:var(--brand)]/20'
							: ''} 
                   {i === activeIndex ? 'ring-1 ring-[color:var(--brand)]/40' : ''}"
					...
					>
						...
						{#if opt === value}
							<svg
								xmlns="http://www.w3.org/2000/svg"
								class="h-4 w-4 text-[color:var(--brand)]"
								viewBox="0 0 24 24"
								fill="currentColor"
							>
