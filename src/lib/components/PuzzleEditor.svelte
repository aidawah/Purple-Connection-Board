<script lang="ts">
	...
	let categoryCount = Math.max(
		MIN,
		Math.min(
			MAX,
			initial.categories?.length ?? (parseInt(initial.size?.split('x')?.[0] || '4') || 4)
		)
	);
	let wordCount = Math.max(
		MIN,
		Math.min(
			MAX,
			initial.categories?.[0]?.words?.length ??
				(parseInt(initial.size?.split('x')?.[1] || '4') || 4)
		)
	);
	type Cat = { name: string; words: string[] };
	let categories: Cat[] = (initial.categories ?? []).map((c) => ({
		name: c.title,
		words: [...(c.words ?? [])]
	}));
	...
	function ensureGrid() {
		if (categories.length !== categoryCount) {
			const next = Array.from(
				...
				(_, i) => categories[i] ?? { name: '', words: [] as string[] }
			);
			next.forEach(
				(c) => (c.words = Array.from({ length: wordCount }, (_, j) => c.words?.[j] ?? ''))
			);
			categories = next;
		}
		if ((categories[0]?.words?.length ?? 0) !== wordCount) {
			categories = categories.map((c) => ({
				...c,
				words: Array.from({ length: wordCount }, (_, j) => c.words?.[j] ?? '')
			}));
		}
	}
	function resetGrid() {
	...
	function mergeWords(existing: string[], incoming: string[], targetLen: number) {
		...
		for (let i = 0; i < targetLen; i++) {
			...
			while (ptr < incoming.length) {
				...
				if (cand && !seen.has(cand.toLowerCase())) {
					filled[i] = cand;
	...
	async function generateCategories() {
		...
		generating = true;
		status('Generating categories…', 'info');
		try {
			const payload = {
				title,
				categoryCount,
			...
			const data: { title?: string; categories?: ApiCategory[] } = await res
				.json()
				.catch(() => ({}) as any);
	...
	async function generateWords() {
		...
		try {
			...
			(data.categories ?? []).forEach((c) =>
				byName.set((c?.name ?? '').trim(), Array.isArray(c?.words) ? c.words : [])
			);
			categories = categories.map((c) => ({
				...c,
				words: mergeWords(c.words ?? [], byName.get(c.name.trim()) ?? [], wordCount)
			}));
	...
	async function submit() {
		...
		if (!title.trim()) {
			status('Please enter a title.', 'error');
			return;
		}
		const anyEmpty = categories.some((c) => !c.name.trim() || c.words.some((w) => !w.trim()));
		if (anyEmpty) {
			status('Please complete all category names and every word box.', 'error');
			return;
		}
...
<div class="w-full" style="--brand: {brand}">
	<div class="mx-auto grid max-w-7xl grid-cols-1 gap-6 px-3 py-6 sm:px-6 xl:grid-cols-[1fr_1fr]">
		<!-- LEFT -->
		<section class="space-y-6">
			<div
				class="rounded-2xl border border-zinc-200 bg-white/90 p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/80"
			>
				<label class="text-sm font-medium text-zinc-700 dark:text-zinc-200" for="title"
					>Puzzle title</label
				>
				<input
					id="title"
					class="mt-2 h-11 w-full rounded-lg border border-zinc-300 bg-white px-3 text-zinc-900 outline-none focus:ring-2 focus:ring-[color:var(--brand)] dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
					bind:value={title}
					placeholder="e.g. Hospital Departments"
				/>
			</div>

			<!-- sliders -->
			<div
				class="space-y-5 rounded-2xl border border-zinc-200 bg-white/90 p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/80"
			>
				<div>
					<div class="flex items-center justify-between">
						<label class="text-sm font-medium text-zinc-700 dark:text-zinc-200" for="catRange"
							>Categories</label
						>
					...
					<input
						...
						class="w-full accent-[color:var(--brand)]"
					/>
				</div>
				<div>
					<div class="flex items-center justify-between">
						<label class="text-sm font-medium text-zinc-700 dark:text-zinc-200" for="wordRange"
							>Words per category</label
						>
				...
				<div class="flex flex-wrap gap-3 pt-1">
					<button
						class="rounded-xl border border-zinc-300 bg-white px-4 py-2 text-zinc-800 shadow-sm hover:bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
						on:click={() => {
							resetGrid();
							title = '';
						}}>Clear</button
					...
					{#if enableGenerate}
						<button
							class="rounded-xl border border-zinc-300 bg-white px-4 py-2 text-zinc-800 shadow-sm hover:bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
							on:click={generateCategories}
							disabled={generating}>Generate categories</button
						>
						<button
							class="rounded-xl border border-zinc-300 bg-white px-4 py-2 text-zinc-800 shadow-sm hover:bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
							on:click={generateWords}
							disabled={generating}>Generate words</button
						>
					{/if}
					<div class="flex-1"></div>
					<button
						class="rounded-xl bg-[color:var(--brand)] px-4 py-2 text-white shadow-sm hover:opacity-90 disabled:opacity-60"
						on:click={submit}
						disabled={busy || generating}>{busy ? 'Saving…' : submitLabel}</button
					>
				</div>
			</div>
			...
			<!-- grid -->
			<div
				class="space-y-4 rounded-2xl border border-zinc-200 bg-white/90 p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/80"
			>
				{#each categories as cat, ci}
					<div class="grid grid-cols-1 items-start gap-3 sm:grid-cols-5">
						<div class="sm:col-span-2">
							<label class="text-sm font-medium text-zinc-700 dark:text-zinc-200" for={`cat-${ci}`}
								>Category {ci + 1}</label
							>
							<input
								id={`cat-${ci}`}
						...
						<div class="grid grid-cols-2 gap-2 sm:col-span-3 sm:grid-cols-4">
							{#each cat.words as _, wi}
								<input
									class="h-10 w-full rounded-lg border border-zinc-300 bg-white px-3 text-zinc-900 outline-none focus:ring-2 focus:ring-[color:var(--brand)] dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
		...
		<!-- RIGHT: preview -->
		<section class="space-y-3 self-start xl:sticky xl:top-4">
