<script lang="ts">
  ...
  type Spot = {
    i: number; x: number; y: number; size: number; dx: number; dy: number;
    rotOriginX: number; rotOriginY: number; delay: number; duration: number;
  };
  const SPOT_COUNT = 52;
  ...
  function buildSpots(scale: number, slow: number, smin: number, smax: number): Spot[] {
    ...
    for (let i = 1; i <= SPOT_COUNT; i++) {
      let left = 0, top = 0;
      if (i <= 20)      { left = -25 + i * 12;  top = 50; }
      else if (i <= 40) { left = -255 + i * 12; top = -12; }
      else if (i <= 46) { left = 204;           top = -488 + i * 12; }
      else              { left = -10;           top = -568 + i * 12; }
      raw.push({ i, left, top });
    }
    const xs = raw.map(p => p.left), ys = raw.map(p => p.top);
    ...
    for (const { i, left, top } of raw) {
      ...
      let dx = 0, dy = 0;
      if (i <= 20)      { dx = (-20 + i * 2) * scale * slow; dy =  30 * scale * slow; }
      else if (i <= 40) { dx = (-50 + i * 2) * scale * slow; dy = -30 * scale * slow; }
      else if (i <= 45) { dx =  40 * scale * slow;           dy = (-86 + i * 2) * scale * slow; }
      else              { dx = -40 * scale * slow;           dy = (-99 + i * 2) * scale * slow; }
...
{#if visible}
  <div class="absolute inset-0 pointer-events-none z-10" style={`--pad:${pad}px`}>
    {#each anchors as a}
      <div class="absolute" style={`left:${a.left}; top:${a.top}; transform: translate(${a.tx}, ${a.ty}) rotate(${a.rot}deg);`}>
        {#each spots as s}
...
<style>
  @keyframes fly {
    ...
    to   { opacity: .9; transform: translate(var(--dx), var(--dy)); }
  }
</style>
