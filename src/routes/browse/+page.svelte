<script lang="ts">
	...
	$: filteredPuzzles = allPuzzles.filter((puzzle) => {
		...
		const matchesSearch =
			!q || puzzle.title.toLowerCase().includes(q) || puzzle.description.toLowerCase().includes(q);
		const matchesCategory =
			selectedCategory === 'All' || (puzzle.category ?? 'General') === selectedCategory;
...
<main class="mx-auto max-w-7xl px-4 pt-10 md:pt-12">
	...
	<!-- Filters -->
	<div
		class="mb-8 rounded-xl border border-zinc-200 bg-white p-6 shadow-sm focus-within:ring-1 focus-within:ring-[color:var(--brand)]
		...
		<div class="grid grid-cols-1 gap-4 md:grid-cols-4">
			<div class="md:col-span-2">
				...
				<input
                 ...
                 focus:border-transparent focus:ring-2 focus:ring-[color:var(--brand)] focus:outline-none
			...
			>
				...
				<div class="p-6">
					<div class="mb-3 flex items-start justify-between">
						<h3 class="text-lg leading-tight font-semibold text-zinc-900 dark:text-zinc-100">
							{puzzle.title}
						</h3>
					</div>

					<p
						class="[WebkitLineClamp:2] [WebkitBoxOrient:vertical] mb-4 [display:-webkit-box] overflow-hidden text-sm text-zinc-600 dark:text-zinc-400"
					>
						{puzzle.description}
					</p>
					...
					<div class="mb-4 flex items-center justify-between">
						<div class="flex items-center gap-2">
							<span
								class="rounded px-2 py-1 text-xs font-medium text-zinc-700 ring-1 ring-[color:var(--brand)]/30 ring-inset dark:text-zinc-300"
							>
								{puzzle.category ?? 'General'}
							</span>
							<span
								class={`rounded px-2 py-1 text-xs font-medium ${getDifficultyColor(puzzle.difficulty)}`}
							>
								{puzzle.difficulty}
							</span>
						</div>
	...
	{#if debug.loaded && allPuzzles.length === 0}
		<div class="py-12 text-center">
			...
			<p class="mb-4 text-zinc-600 dark:text-zinc-400">
				If you expected 2 puzzles, check projectId above.
			</p>
		</div>
	{/if}
</main>
