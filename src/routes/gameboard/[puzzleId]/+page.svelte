<script lang="ts">
  ...
  const BRAND = '#14b8a6';
  const CURSOR_SRC = '/demo-cursor.png'; // put image in /static
  ...
  // Best: target tiles by a data attribute we add to GameBoard (see patch below).
  function findWordButton(word: string): HTMLButtonElement | null {
    // If you add data-word in GameBoard, prefer this:
    const btn = document.querySelector<HTMLButtonElement>(`button[data-word="${CSS.escape(word)}"]`);
    if (btn) return btn;

    // Fallback: match by visible text on tile buttons (they have data-selected)
  ...
  async function autoSolve() {
    ...
    // Use the categories from the demo payload returned by the loader.
    for (const cat of (data.puzzle?.categories ?? [])) {
      ...
      // Let GameBoard resolve flip/lock after 4 picks
      await pause(360);
    }
...
<main class="mx-auto max-w-5xl px-4 py-8">
  ...
  <GameBoard puzzleId={data.id} puzzle={data.puzzle} showControls />
  ...
  {#if isDemo}
    ...
    <div
      ...
      style="
        ...
        background-image: url('/demo-cursor.png');
