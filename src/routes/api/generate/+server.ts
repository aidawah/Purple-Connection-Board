...
const DISALLOWED_CATEGORY_NAMES = [
  'colors','colour','fruits','fruit','animals','animal','vehicles','vehicle','shapes','shape',
  'countries','country','months','month','days','day','numbers','number','sports','sport',
  'musical instruments','instrument','foods','food','jobs','job','flowers','flower'
].map(s=>s.toLowerCase());

// Mild word bans to deter repeats across sets
const DISALLOWED_WORDS = [
  'red','blue','green','apple','banana','cat','dog','car','truck','circle','square'
].map(s=>s.toLowerCase());

// If the caller doesn’t give hints, we rotate through varied domains to increase diversity
const THEME_POOL = [
  'healthcare workflows','medical imaging','hospital departments','bioinformatics',
  'construction trades','building materials','HVAC components',
  'finance & accounting terms','supply chain','transport logistics',
  'software testing','frontend frameworks','networking hardware','cloud services',
  'culinary techniques','coffee roasting','baking science',
  'music production','film editing','lighting design',
  'gardening tools','soil science','botany (non-animal)',
  'astronomy (non-astrology)','geology','oceanography',
  'linguistics','etymology','rhetorical devices',
  'board game mechanics','video game genres (specific)'
];
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const hints = (themeHints && themeHints.length)
      ? themeHints
      : pickRandomHints(2);
    ...
    if (allAvoidCats.length) userLines.push(`Avoid category names (do NOT use): ${allAvoidCats.join(' | ')}`);
    if (allAvoidWords.length) userLines.push(`Avoid words (do NOT use): ${allAvoidWords.join(' | ')}`);
    ...
    if (mode === 'single' && targetIndex > -1 && categories[targetIndex]) {
      ...
      categories.forEach((c, i) => {
        userLines.push(`${i}. name="${c.name || 'Untitled'}" | seed=[${(c.seedWords || []).join(', ')}] | need=${c.need ?? 0}`);
      });
    }
    ...
    userLines.push('- Categories must be meaningfully distinct and NOT generic schoolbook sets (e.g., colors, fruits, animals, vehicles).');
    userLines.push('- For missing-fill, keep existing seed words and add distinct new ones to reach the required length.');
    ...
    while (attempt < maxAttempts) {
      ...
      const completion = await client.chat.completions.create({
        ...
        messages: [
          { role: 'system', content: 'You create word-group puzzles like NYT Connections. Return ONLY compact JSON conforming to the schema. No prose.' },
          { role: 'user', content: userLines.join('\n') }
        ]
      });

      const candidates = (completion.choices ?? []).map((ch) => safeJSON(ch?.message?.content ?? '{}'));
...
function noveltyScore(
  ...
  for (const c of out.categories ?? []) {
    ...
    for (const w of c?.words ?? []) {
      ...
      if (seen.has(lw)) score -= 1; else { seen.add(lw); score += 1; }
    }
  }
  return score;
}
...
function pickRandomHints(k: number) {
  ...
  while (out.length < k && used.size < THEME_POOL.length) {
    ...
    if (!used.has(i)) { used.add(i); out.push(THEME_POOL[i]); }
  }
  return out;
}
